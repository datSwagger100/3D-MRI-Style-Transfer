...
class GANLoss(nn.Module):
    ...
    def __call__(self, prediction, target_is_real):
        ...
        if self.gan_mode in ['lsgan', 'vanilla']:
            ...
            eps = 1e-6
            if self.loss(prediction, target_tensor).isnan(): 
                loss = eps
            else: 
                pass
                #loss = self.loss.item()
            loss = eps + self.loss(prediction, target_tensor)
...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        # ! Manually Set number of downsampling layer of UNet here.
    ...
    def forward(self, input):
        ...
        # print("UNET GENERATOR INFO")
        # print("Current input channel", self.input_nc)
        # print("Current output channel: ", self.output_nc)
        # print("Num of down sampling layers: ", self.num_downs)   
        return self.model(input)
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("--- UNET SKIPPING CONNECTION INFO ---")
        # print("Current outer channel: ", self.outer_nc)
        # print("Current inner channel ", self.inner_nc)
        # print("Current input channel ", self.input_nc)  

        # print("--- UNET x and model(x) cat INFO ---")
        # print("shape of x is: ", np.shape(x))
        # print("shape of model(x) is: ", np.shape(self.model(x)))
        ...
        if self.outermost:
            ...
            #return torch.stack((x, self.model(x)), dim = 1)
